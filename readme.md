# Using Lean 4 with Overleaf

This repository demonstrates a workflow for integrating formal theorem proving into LaTeX publishing using Overleaf and GitHub Actions.

While Overleaf does not natively compile theorem prover code, Overleaf's **GitHub Sync** feature allows authors to execute Lean 4 verification checks on GitHub runners and pull the resulting proof verification results directly back into their LaTeX project.

---

## Overview

When publishing mathematical results, three distinct layers of abstraction work in tandem:

1. **Human-Oriented Proof ( found in `main.tex`):** Shows an example of the typical proof by induction of the identity.


2. **Tactic Script (`gauss.lean`):** A simple proof script for the Gauss sum that closely mirrors the human-oriented proof. A second version, gauss-tactic.lean provides a recipe that uses high-level Lean tactics (`induction`, `simp`, `linarith`) that directs the Lean elaborator on how to synthesize a formal proof.


3. **Elaborated Proof Term & Receipt (`lean_output.txt`):** The formal $\lambda$-calculus term checked by Lean's trusted kernel. 


---

## Artifacts in the project

| File / Artifact | Purpose |
| --- | --- |
| **`main.tex`** | Primary LaTeX paper presenting the Gauss summation identity. It includes `gauss.lean` and `lean_output.txt` using the `minted` package.|
| **`gauss.lean`** |Lean 4 source that uses tactics, defining the recursive function `sum_to` and proving using induction. |
| **`gauss-tactic.lean`**  | Lean 4 source that uses tactics, defining the recursive function `sum_to` and proving `gauss_sum` using `linarith`. |
| **`lakefile.lean`** | Package manifest specifying project configurations and pinned dependencies (e.g., Mathlib at `v4.7.0`).|
| **`lean-toolchain`** | Pins the exact Lean 4 version (`leanprover/lean4:v4.7.0`) used across local and CI environments. |
| **`lake-manifest.json`** | Auto-generated lockfile locking exact dependencies for build reproducibility. |
| **`lean_output.txt`** | The execution log generated by running `lake env lean gauss.lean` |
| **`.github/workflows/init_lean.yml`** | A one-time bootstrap workflow that installs `elan`, runs `lake update`, and generates the initial `lake-manifest.json`.|
| **`.github/workflows/lean_action.yml`** | The  workflow that runs `leanprover/lean-action@v1` on demand to re-verify proofs and commit updated receipts.|

---

## Software used

* **Elan:** Lean's version manager (similar to `rustup`). Reads `lean-toolchain` and provisions the correct compiler release.
* **Lake:** Lean 4's build system and package manager. Reads `lakefile.lean` and resolves external libraries.
* **Lean 4 Engine:** Comprises the *Elaborator* (which expands  `gauss.lean`) and the *Trusted Kernel* (which validates the synthesized proof terms).
* **Mathlib4:** The  mathematics library providing tactical automation such as `linarith`. Only actually used for gauss-tactic.lean.
* **`leanprover/lean-action@v1`:** provided GitHub Action that handles environment setup, caching, and execution.



---

## How it works

1. **Write & Edit:** Authors edit `main.tex` and `gauss.lean` directly in Overleaf


2. **Trigger Verification:** You need to explicitly run the action on the GitHub side (`workflow_dispatch`) `.github/workflows/lean_action.yml`.


3. **Execute & Commit:** GitHub Actions executes Lean, captures `#print gauss_sum` or `#check gauss_sum` output into `lean_output.txt`, and commits the this output back to the repository.


4. **Sync Back:** In Overleaf, use GitHub Sync to fetch the updated `lean_output.txt`
